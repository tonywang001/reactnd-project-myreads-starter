...
class BooksApp extends React.Component {
  ...
  onMove(id, shelf) {
    ...
    this.setState((state) => {
      return {
        shelfList: state.shelfList.map((prevShelf) => {
          if (prevShelf.bookList.includes(id)) {
            const index = prevShelf.bookList.indexOf(id);
            prevShelf.bookList.splice(index, 1);
          }
          if (prevShelf.id === shelf) {
            prevShelf.bookList.push(id);
          }
          return prevShelf;
        })
      };
  ...
  // Get the book list by id list
  getBookListByIdList(idList) {
    return this.state.bookList.filter((book) => 
      idList.includes(book.id)
    );
  };
              ...
              <div>
                {shelfList && shelfList.map((shelf) => (
                  <BookShelf key={shelf.id} 
                             title={shelf.title} 
                             bookList={this.getBookListByIdList(shelf.bookList)}
